...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔬 **`/proof-checker` two new opt-in flags: `--deep-fix` + `--restatement-check`** ([#188](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/188), [#189](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/189), [SKILL.md](skills/proof-checker/SKILL.md)). `--deep-fix` augments Phase 1 reviewer prompt with a structured repair-grade plan (corrected_statement / changed_equations / minimal_tex_patch_plan / closure_tests + algebra sanity for Schur / quadratic-form). `--restatement-check` adds Phase 3.6 cross-location theorem drift detector (6 drift signatures: conditional_loss / scope_change / quantifier_loss / regime_envelope_change / constant_change / variable_rename). **Zero behavior change** when flags unset; advisory only (verdict / reason_code / summary unchanged). Both built via fast-path delegated-agent + reviewer-fixup workflow (one PR per flag, sibling-branch rebase to combine `argument-hint`)
