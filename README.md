...
- **2026-05-01** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔍 **Gemini + OpenAlex literature sources for `/research-lit`** ([#175](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/175), community contribution by [@stdAri](https://github.com/stdAri)). Two new opt-in sources for Workflow 1 idea discovery: [`/gemini-search`](skills/gemini-search/SKILL.md) (AI-powered broad discovery via [`jamubc/gemini-mcp-tool`](https://github.com/jamubc/gemini-mcp-tool) MCP with `@google/gemini-cli` fallback — decomposes topics into sub-problems, aliases, and naming variants) and [`/openalex`](skills/openalex/SKILL.md) (open citation graph covering 250M+ works via [`tools/openalex_fetch.py`](tools/openalex_fetch.py) — institutional affiliations, NSF/NIH funding data, no API key required). Routed through `/research-lit "topic" — sources: gemini` or `— sources: openalex`. **Zero behavior change for existing users** — both are explicitly excluded from the default `all` set; `/research-lit "topic"` without source overrides keeps prior behavior. De-dup rules: prefer S2 for citation counts, arXiv for PDF metadata, OpenAlex for institution/funding (merged into richer record). Maintainer fixup commits before merge: corrected the `@google/gemini-cli` npm package name (PR originally shipped `@anthropic-ai/gemini-cli` by mistake) and added `try/except ImportError` + bash preflight so users without the `requests` package skip OpenAlex silently instead of seeing a `ModuleNotFoundError` stack trace. Pre-merge safety reviewed via Codex MCP cross-model audit (`gpt-5.5` xhigh) covering default-source contamination, cross-source side effects, dedup regression, hard-fail dependency handling, downstream skill propagation, and 3-way merge sanity
