...
- **2026-05-01** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔍 **`/research-lit` 新增 Gemini + OpenAlex 文献源** ([#175](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/175)，社区贡献 [@stdAri](https://github.com/stdAri))。Workflow 1 idea discovery 多了两个 opt-in 文献源：[`/gemini-search`](skills/gemini-search/SKILL.md)（通过 [`jamubc/gemini-mcp-tool`](https://github.com/jamubc/gemini-mcp-tool) MCP，CLI fallback 走 `@google/gemini-cli`，把主题分解成子问题、别名、命名变体做广覆盖检索）与 [`/openalex`](skills/openalex/SKILL.md)（基于 [`tools/openalex_fetch.py`](tools/openalex_fetch.py) 调 OpenAlex 开放引用图，2.5 亿+ 条目，带机构归属、NSF/NIH 资助信息，免 API key）。通过 `/research-lit "topic" — sources: gemini` 或 `— sources: openalex` 显式触发。**对老用户零行为变化**——两个新源都被显式排除在默认 `all` 集合之外；不带 `— sources:` 参数跑 `/research-lit "topic"` 行为完全不变。去重规则：S2 的引用数优先、arXiv 的 PDF 元数据优先、OpenAlex 的机构/资助信息独占性合并到 richer record。Merge 前 maintainer 加了两个 fixup commit：修正 `@google/gemini-cli` 的 npm 包名（PR 原稿误写 `@anthropic-ai/gemini-cli`）、为 OpenAlex 加 `try/except ImportError` + bash preflight，避免没装 `requests` 包的用户看到 `ModuleNotFoundError` 栈而是 silent skip。Merge 前的安全 review 由 Codex MCP 跨家族审（`gpt-5.5` xhigh）覆盖默认源污染、跨源副作用、去重回归、硬依赖处理、下游 skill 传播、3-way merge 语义六个维度
