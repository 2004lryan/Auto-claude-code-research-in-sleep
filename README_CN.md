...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔬 **`/proof-checker` 加两个 opt-in flag：`--deep-fix` + `--restatement-check`** ([#188](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/188), [#189](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/189), [SKILL.md](skills/proof-checker/SKILL.md))。`--deep-fix` 让 Phase 1 reviewer 输出 repair-grade 修复计划（corrected_statement / changed_equations / minimal_tex_patch_plan / closure_tests + Schur/quadratic-form 代数 sanity）。`--restatement-check` 加 Phase 3.6 跨位置定理飘移检测（6 类 drift：conditional_loss / scope_change / quantifier_loss / regime_envelope_change / constant_change / variable_rename）。**flag 不传时零行为变化**；advisory only（不改 verdict / reason_code / summary）。两个都走 fast-path delegated-agent + reviewer-fixup（每 flag 一个 PR，sibling-branch rebase 合并 `argument-hint`）
